...
domain_list_de = ['Schreiben', 'Rollenspiel', 'Extraktion', 'Argumentation', 'Mathematik', 'Programmierung', 'MINT-Wissen', 'Geistes- und Sozialwissenschaftliches Wissen']
...
qgen_command_dict_de = {
'Schreiben': 'Es sollte eine Benutzeranfrage sein, die das LLM auffordert, etwas zu schreiben.',
'Rollenspiel': 'Es sollte ein Szenario vorschlagen, in dem der Chatbot eine bestimmte Rolle/Person nachahmt. Geben Sie alle notwendigen Anweisungen und Anforderungen für seine Antwort. Senden Sie dann eine Anfangsanfrage zum Vervollständigen.',
'Extraktion': 'Es sollte aus zwei Teilen bestehen: Frage und Kontext. Die Frage sollte die Fähigkeit des Chatbots testen, Informationen aus dem gegebenen Kontext korrekt zu verstehen und zu extrahieren. Entwerfen und stellen Sie selbst einen neuen Kontext bereit.',
'Argumentation': 'Es sollte eine spezifische Frage sein, die darauf ausgelegt ist, die Argumentationsfähigkeiten des LLM zu testen.',
'Mathematik': 'Es sollte eine spezifische Frage sein, die darauf ausgelegt ist, die mathematischen Fähigkeiten des LLM zu testen.',
'Programmierung': 'Es sollte eine spezifische Frage sein, die darauf ausgelegt ist, die Programmierfähigkeiten des LLM zu testen.',
'MINT-Wissen': 'Es sollte eine spezifische Frage sein, die darauf ausgelegt ist, das MINT-Wissen des LLM zu testen.',
'Geistes- und Sozialwissenschaftliches Wissen': 'Es sollte eine spezifische Frage sein, die darauf ausgelegt ist, das geistes- und sozialwissenschaftliche Wissen des LLM zu testen.',
}
...
将答案作为整数的JSON数组返回。
"""
extraction_example_de = """
Frage:
Bewerten Sie die folgenden Filmkritiken auf einer Skala von 1 bis 5, wobei 1 sehr negativ, 3 neutral und 5 sehr positiv ist:
Kontext:
Dieser am 18. November 2019 veröffentlichte Film war phänomenal. Die Kameraführung, das Schauspiel, die Handlung - alles war erstklassig.
Noch nie war ich so enttäuscht von einem Film. Die Handlung war vorhersehbar und die Charaktere waren eindimensional. Meiner Meinung nach ist dieser Film der schlechteste, der 2022 veröffentlicht wurde.
Der Film war in Ordnung. Es gab einige Teile, die ich genossen habe, aber es gab auch Teile, die sich glanzlos anfühlten. Dies ist ein Film, der im Februar 2018 veröffentlicht wurde und ziemlich gewöhnlich zu sein scheint.
Geben Sie die Antwort als JSON-Array von ganzen Zahlen zurück.
"""
...
qgen_example_dict_de = {
'Schreiben': 'Verfassen Sie einen fesselnden Reiseblog-Beitrag über eine kürzliche Reise nach Hawaii, der kulturelle Erfahrungen und Sehenswürdigkeiten hervorhebt, die man unbedingt gesehen haben muss.',
'Rollenspiel': 'Stellen Sie sich vor, Sie seien Elon Musk in allen folgenden Gesprächen. Sprechen Sie so weit wie möglich wie Elon Musk. Warum müssen wir zum Mars fliegen?',
'Extraktion': extraction_example_de,
'Argumentation': 'Stellen Sie sich vor, Sie nehmen an einem Rennen mit einer Gruppe von Menschen teil. Wenn Sie gerade die zweite Person überholt haben, welche Position haben Sie jetzt? Wo ist die Person, die Sie gerade überholt haben?',
'Mathematik': 'Die Eckpunkte eines Dreiecks befinden sich an den Punkten (0, 0), (-1, 1) und (3, 3). Wie groß ist die Fläche des Dreiecks?',
'Programmierung': 'Entwickeln Sie ein Python-Programm, das alle Textdateien in einem Verzeichnis liest und die Top-5-Wörter mit der höchsten Anzahl an Vorkommen zurückgibt.',
'MINT-Wissen': 'Im Bereich der Quantenphysik, was ist Superposition und wie hängt sie mit dem Phänomen der Quantenverschränkung zusammen?',
'Geistes- und Sozialwissenschaftliches Wissen': 'Geben Sie Einblicke in die Korrelation zwischen wirtschaftlichen Indikatoren wie BIP, Inflation und Arbeitslosenquoten. Erklären Sie, wie Fiskal- und Geldpolitik diese Indikatoren beeinflussen.',
}
...
question_generation_instruction_de = """Ihnen wurde die Aufgabe übertragen, eine Reihe von //NUM// verschiedenen Benutzeranfragen an einen Chat-Assistenten zum Thema //DOMAIN// zu entwerfen. Bitte befolgen Sie strikt diese 6 Regeln für die Frage:
Die Frage sollte realistisch sein, also eine, die ein Benutzer im echten Leben stellen könnte. Folgen Sie dem Format der Beispielanfrage. //QGEN_COMMAND_DOMAIN// 2. Sie sollte vom Chatbot selbst ohne zusätzliche Eingaben beantwortet werden können. 3. Sie müssen die Anfragen so VIELFÄLTIG wie möglich generieren. 4. Fügen Sie KEINE anderen Wörter außer der Anfrage selbst hinzu. 5. Die Frage sollte komplex und schwierig sein und ein tiefgreifendes Verständnis sowie eine gründliche Analyse des Themas erfordern.
Jede Frage in einer Zeile, fügen Sie die laufende Nummer in Klammern (z.B. "(1).", "(2).") vor jeder Frage hinzu.
Beispielanfrage: //QGEN_EXAMPLE_DOMAIN//"""
...
actions_de = ['<antworten>', '<nachdenken>', '<kritisieren>', '<einwenden>']

init_user_input = 'initial user input:'
init_user_input_zh = '初始用户输入:'
init_user_input_de = 'Anfängliche Benutzereingabe:'
...
candidate_instruction_de = """Sie sind ein hilfreicher Assistent, der genaue Antworten auf Benutzeranfragen gibt. Als erfahrener Assistent folgen Sie den Anfragen des Benutzers und geben so zuverlässige Antworten wie möglich. Sie skizzieren Ihre Gründe für die Antwort, um es den Benutzern leicht zu machen, diese zu verstehen. Während Sie die wichtigen Details in den Antworten beibehalten, zielen Sie darauf ab, prägnante und direkte Antworten zu geben, ohne unnötig weitschweifig zu sein.
Dies ist eine kompetitive Chatbot-Arena. Sie treten in einer Debatte gegen einen anderen Chatbot-Assistenten an und werden von einem Komitee nach Faktoren wie Hilfsbereitschaft, Relevanz, Genauigkeit, Tiefe und Kreativität beurteilt. Nach der Beantwortung der anfänglichen Benutzereingabe werden Sie sich in einer mehrrundigen Debatte mit Ihrem Gegner engagieren. Hier sind Ihre Aktionen:
<nachdenken>: Denken Sie Schritt für Schritt, um die Frage zu analysieren oder Ihre Strategie in der Debatte zu planen. Dies ist für den Gegner verborgen. Denken Sie nur wenn nötig nach und halten Sie es prägnant.
<antworten>: Antworten Sie so genau wie möglich auf die Benutzereingabe.
<kritisieren>: Kritisieren Sie die Schwächen der Antwort Ihres Gegners.
<einwenden>: Zielen Sie auf die Schwächen Ihres Gegners ab. Geben Sie eine potenzielle Folge-Benutzereingabe, die der Gegner möglicherweise nicht beantworten kann. Die Eingabe sollte prägnant beantwortet werden können und sich auf Variationen oder Motivationen seiner vorherigen Antwort konzentrieren. Generieren Sie nur eine Eingabe. Seien Sie vernünftig. Vermeiden Sie es, zu spezifisch oder repetitiv zu werden. Machen Sie KEINEN Einwand, wenn Sie die Antwort des Gegners NICHT SEHEN!
Folgen Sie dem Aktionsleitfaden strikt."""
...
action_prompts_de = {
'<respond>': "Aktionsleitfaden: nur <antworten> einschließen. Verwenden Sie <nachdenken> bei Bedarf. Schließen Sie Ihre gesamte Antwort innerhalb von 300 Wörtern ab, einschließlich <nachdenken>. UMSCHLIESSEN SIE JEDE AKTION MIT IHREN JEWEILIGEN TAGS!",
'<criticize>_<raise>': "Aktionsleitfaden: sowohl <kritisieren> als auch <einwenden> einschließen. Verwenden Sie <nachdenken> bei Bedarf. Schließen Sie Ihre gesamte Antwort innerhalb von 300 Wörtern ab, einschließlich <nachdenken>. UMSCHLIESSEN SIE JEDE AKTION MIT IHREN JEWEILIGEN TAGS!",
'<respond>_<criticize>_<raise>': "Aktionsleitfaden: alle Aktionen <antworten>, <kritisieren> und <einwenden> einschließen. Verwenden Sie <nachdenken> bei Bedarf. Schließen Sie Ihre gesamte Antwort innerhalb von 600 Wörtern ab, einschließlich <nachdenken>. UMSCHLIESSEN SIE JEDE AKTION MIT IHREN JEWEILIGEN TAGS!",
}
...
action_prompts_writing_de = {
'<respond>': "Aktionsleitfaden: nur <antworten> einschließen. Verwenden Sie <nachdenken> bei Bedarf. Schließen Sie Ihre gesamte Antwort innerhalb von 400 Wörtern ab, einschließlich <nachdenken>. UMSCHLIESSEN SIE JEDE AKTION MIT IHREN JEWEILIGEN TAGS!",
'<criticize>_<raise>': "Aktionsleitfaden: nur <kritisieren> und <einwenden> einschließen. Verwenden Sie <nachdenken> bei Bedarf. Verwenden Sie nicht <antworten>. Schließen Sie Ihre gesamte Antwort innerhalb von 400 Wörtern ab, einschließlich <nachdenken>. UMSCHLIESSEN SIE JEDE AKTION MIT IHREN JEWEILIGEN TAGS!",
'<respond>_<criticize>_<raise>': "Aktionsleitfaden: alle Aktionen <antworten>, <kritisieren> und <einwenden> einschließen. Verwenden Sie <nachdenken> bei Bedarf. Schließen Sie Ihre gesamte Antwort innerhalb von 800 Wörtern ab, einschließlich <nachdenken>. UMSCHLIESSEN SIE JEDE AKTION MIT IHREN JEWEILIGEN TAGS!",
}

need_extra_space_cats = ['writing', 'roleplay', 'coding', 'humanities/social science knowledge']
need_extra_space_cats_zh = ['编程']
need_extra_space_cats_de = ['Schreiben', 'Rollenspiel', 'Programmierung', 'Geistes- und Sozialwissenschaftliches Wissen']
...
missing_actions_prompts_de = 'Generieren Sie nur //ACTIONS_UNDONE//! UMSCHLIESSEN SIE SIE MIT TAGS!'

opponent_response = 'Opponent\'s Response:'
opponent_response_zh = '对手的回答:'
opponent_response_de = 'Antwort des Gegners:'
#######################################################
################### JUDGE PROMPTS #####################
#######################################################
...
round_instruction_de = '[[Runde //NUM//]]'

assistant = 'Assistant //ROLE//'
assistant_zh = '//ROLE//助手'
assistant_de = 'Assistent //ROLE//'
...
ref_answer_prompt_de = 'Bitte geben Sie eine präzise und knappe Antwort auf die folgende Frage. Fügen Sie nichts anderes hinzu.\n'
...
judge_instruction_de = "Dies ist eine Chatbot-Arena. Zwei KI-Assistenten hatten eine mehrründige Debatte darüber, wer hilfsbereiter ist. Bitte agieren Sie als unparteiischer Richter und bewerten Sie die Fähigkeiten der beiden KI-Assistenten. Sie sollten den Assistenten wählen, der Anweisungen besser befolgt und Fragen besser beantwortet. Ihre Bewertung sollte Faktoren wie Hilfsbereitschaft, Relevanz und Genauigkeit berücksichtigen. Beginnen Sie Ihre Bewertung, indem Sie die Antworten der beiden Assistenten vergleichen und eine kurze Erklärung abgeben. Vermeiden Sie jegliche Positionsvoreingenommenheit und stellen Sie sicher, dass die Reihenfolge, in der die Antworten präsentiert wurden, Ihre Entscheidung nicht beeinflusst. Lassen Sie sich bei Ihrer Bewertung NICHT von der LÄNGE der Antworten beeinflussen, wählen Sie stattdessen die direkte und prägnante Antwort anstatt der unnötig ausführlichen. Wenn die beiden Kandidaten gleich gut abschneiden, wählen Sie die KÜRZERE Antwort. Bevorzugen Sie keine bestimmten Namen der Assistenten. Seien Sie so objektiv wie möglich. Nachdem Sie Ihre Erklärung prägnant innerhalb von 200 Wörtern abgegeben haben, geben Sie Ihr endgültiges Urteil strikt nach folgendem Format aus: '[[A]]', wenn Assistent A besser ist, '[[B]]', wenn Assistent B besser ist, und '[[Unentschieden]]' bei einem Unentschieden. Schließen Sie Ihre Beurteilung innerhalb von 300 Wörtern ab."
...
judge_debate_instruction_de = "Unten finden Sie die Bewertungen der anderen Juroren im Komitee. Bitte lesen Sie diese und entscheiden Sie, ob Sie Ihre Bewertung anpassen oder Ihr ursprüngliches Urteil beibehalten möchten. Nachdem Sie Ihre Erklärung abgegeben haben, geben Sie Ihr endgültiges Urteil strikt nach folgendem Format aus: '[[A]]', wenn Assistent A besser ist, '[[B]]', wenn Assistent B besser ist, und '[[Unentschieden]]' bei einem Unentschieden. Schließen Sie Ihre Beurteilung innerhalb von 300 Wörtern ab."
...
judge_debate_start_de = 'Oben sind die Antworten der anderen Juroren. Bitte geben Sie jetzt Ihre Beurteilung ab:'
...
judge_use_ref_answer_instruction_de = 'Konzentrieren Sie sich mehr auf die Genauigkeit der Antworten. Hier ist eine Referenzantwort, die Sie verwenden können:'
...
class Prompter:
    def __init__(self, lang='en'):
        ...
        if self.lang == 'en':
            ...
            self.domain_list = domain_list_de
            self.qgen_command_dict = qgen_command_dict_de
            self.qgen_example_dict = qgen_example_dict_de
            self.question_generation_instruction = question_generation_instruction_de
            
            self.candidate_instruction = candidate_instruction_de
            self.init_user_input = init_user_input_de
            self.missing_actions_prompts = missing_actions_prompts_de
            
            self.actions = actions_de
            self.action_prompts = action_prompts_de
            self.action_prompts_writing = action_prompts_writing_de
            self.need_extra_space_cats = need_extra_space_cats_de
            self.opponent_response = opponent_response_de
            
            self.round_instruction = round_instruction_de
            self.assistant = assistant_de
            self.judge = judge_de 
            self.tie = tie_de
            self.ref_answer_prompt = ref_answer_prompt_de
            self.judge_instruction = judge_instruction_de
            self.judge_debate_start = judge_debate_start_de
            self.judge_debate_instruction = judge_debate_instruction_de
            self.judge_use_ref_answer_instruction = judge_use_ref_answer_instruction_de
